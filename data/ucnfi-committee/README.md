...
`activity/` is the daily/weekly heartbeat between full enrichment passes — a record of what each member has been publishing or saying about AI in public. It is **generated, not hand-edited**: a GitHub Actions workflow runs `npm run scan:daily` once a day (RSS/Atom + arXiv + Claude web search via the UCSD TritonAI LiteLLM proxy) and `npm run digest:weekly` once a week (same LiteLLM-proxied Claude), and commits both back to this directory.
