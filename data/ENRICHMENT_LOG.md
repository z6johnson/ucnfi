...
- **v0.8.1** — UCSD enterprise platform correction (2026-06-03). Corrected the framing that TritonGPT is a vendor contract on par with ChatGPT Enterprise/ZotGPT. TritonGPT is built and operated in-house by UC San Diego (IT Services + SDSC) on Meta Llama open-source models, and UCSD has licensed the platform to other institutions that rebrand it: UC Berkeley (BearGPT), San Diego State (SDSU-GPT), UC Agriculture & Natural Resources, and Fairleigh Dickinson University. Separately, UCSD also licenses commercial tools (ChatGPT, Microsoft Copilot, P3-approved) alongside TritonGPT. Updated `infrastructure.has_enterprise_ai_platform` and `infrastructure.enterprise_platform_name` for uc_san_diego; added source ucsd-17 (EdTech Magazine, 2025-05). Note: UC Merced was not confirmed as a TritonGPT licensee — it is separately pursuing ChatGPT EDU.
